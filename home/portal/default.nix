...
# memorized: http://design.localhost instead of http://127.0.0.1:5174.
#
...
# `curl -H "Host: design.localhost" 127.0.0.1`. macOS allows unprivileged
    ...
    ${lib.concatStrings (lib.mapAttrsToList (name: port: ''
      http://${name}.localhost:80 {
        reverse_proxy 127.0.0.1:${toString port} {
          # Loopback services tend to allowlist the Host header and
          # 403 anything else; present the address they expect. Browser
